...
const apodoCampo = document.getElementById("tituloNav");
var apodo = localStorage.getItem("apodo");
var id = localStorage.getItem("id");
apodoCampo.innerHTML += `${apodo}`;
...
let enviarDatos = (objeto) => {
  ...
  urlencoded.append("idUsuario", objeto.idUsuario);
  ...
  fetch("http://localhost:4000/api/publicar/publicar", requestOptions)
...
let traerDatos = async (objeto) => {
  ...
  try {
    const response = await fetch(
      "http://localhost:4000/api/publicar/publicar/ver",
      requestOptions
    );
